...
void threadfunc(int idx, vector<wordindex>&results, string filename, string word)
{
    wordindex file;
...
int run_workers(vector<wordindex>& files, vector<string> filenames,
    ...
    if (files_size - start_pos >= num_threads) {
        num_threads = num_threads;
    }
    else {
        num_threads = files_size - start_pos;
    }

    vector<wordindex>results;
    results.resize(num_threads);
    std::vector<std::thread> th;
    th.reserve(num_threads);
    for(int i = 0; i < num_threads; i++)
    {
        th.emplace_back(threadfunc, i, std::ref(results), filenames[start_pos], word);
        start_pos += 1;
    }
    ...
    for(int i = 0; i != num_threads; ++i) {
        num_occurrences += results[i].indexes.size();
        files.push_back(results[i]);
    }
